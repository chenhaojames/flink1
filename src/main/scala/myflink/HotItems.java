...
public class HotItems {
    ...
    public static void main(String[] args) throws Exception {
        ...
        URL fileUrl = HotItems.class.getClassLoader().getResource("hotItem.txt");
        Path filePath = Path.fromLocalFile(new File(fileUrl.toURI()));
        // 抽取 UserBehavior 的 TypeInformation，是一个 PojoTypeInfo
        PojoTypeInfo<UserBehavior> pojoType = (PojoTypeInfo<UserBehavior>) TypeExtractor.createTypeInfo(UserBehavior.class);
        // 由于 Java 反射抽取出的字段顺序是不确定的，需要显式指定下文件中字段的顺序
        String[] fieldOrder = new String[]{"userId", "itemId", "categoryId", "behavior", "timestamp"};
        TextValueInputFormat textValueInputFormat = new TextValueInputFormat(filePath);
        ...
        env
                ...
                .assignTimestampsAndWatermarks(new AscendingTimestampExtractor<UserBehavior>() {
                    ...
                    public long extractAscendingTimestamp(UserBehavior userBehavior) {
                        // 原始数据单位秒，将其转成毫秒
                        return Long.valueOf(userBehavior.timestamp) * 1000;
                    }
                })
                // 过滤出只有点击的数据
    ...
    public static class UserBehavior {
        ...
        public Long itemId;         // 商品ID
        public Integer categoryId;      // 商品类目ID
